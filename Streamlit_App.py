...
def reverse_transformation(prediction):
...
def main():
    ...
    if st.button("Predict Premium"):
        try:
            ...
            preprocessor_instance = preprocessor.__class__(user_input, is_prediction=True)

            processed_data = preprocessor_instance.preprocess()

            st.write("Processed Input Data:", processed_data)

            prediction = model.predict(processed_data)

            original_scale_prediction = reverse_transformation(prediction[0])

            st.success(f"Predicted Premium Amount: ${original_scale_prediction:.2f}")
